...
export default function AllDocuments({ canEdit = false, canUpload = false, canDelete = false, onUseTemplate }) {
  ...
  const handleDownloadTemplate = async (template, format = 'docx') => {
    try {
      console.log('Iniciando download do template parcial:', template.id, 'formato:', format);

      const response = await API.post(
        `/document-layouts/partial-templates/${template.id}/complete`,
        { data: {}, format },
      ...
      const cleanTitle = sanitizeTemplateText(template.title) || 'documento';
      const fileName = format === 'pdf'
        ? `${cleanTitle.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`
        : `${cleanTitle.replace(/[^a-zA-Z0-9]/g, '_')}.docx`;
      ...
      console.error('Erro detalhado ao fazer download do template:', err);
      alert(`Erro ao fazer download do template em ${format.toUpperCase()}: ${err.message}`);
    }
  };
                  ...
                  <div className="card-actions">
                    ...
                    {remainingFields === 0 && (
                      <div className="download-buttons">
                        <button
                          onClick={() => handleDownloadTemplate(item, 'docx')}
                          className="btn btn-primary btn-small"
                        >
                          📄 DOCX
                        </button>
                        <button
                          onClick={() => handleDownloadTemplate(item, 'pdf')}
                          className="btn btn-primary btn-small"
                        >
                          📋 PDF
                        </button>
